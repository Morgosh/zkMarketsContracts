...
contract TwoUserMultisig is IAccount, IERC1271 {
    ...
    uint256 constant withdrawalDelay = 1 seconds; // Safe withdrawal delay of 30 days
    ...
    constructor(address _owner1, address _owner2, address _emergencyWallet) {
        owner1 = _owner1;
        owner2 = _owner2;
        deployedAt = block.timestamp; // Set deployment time
        emergencyWallet = _emergencyWallet; // Set the emergency wallet
    }

    // Withdraw function
    function withdraw() external {
        require(msg.sender == emergencyWallet, "Only the emergency wallet can withdraw funds");
        require(block.timestamp - deployedAt >= withdrawalDelay, "Withdrawal delay has not passed yet");
        
        uint256 balance = address(this).balance;
        payable(emergencyWallet).transfer(balance);
    }

    // For ERC20 token withdrawals
    function withdrawTokens(address tokenContractAddress) external {
        require(msg.sender == emergencyWallet, "Only the emergency wallet can withdraw tokens");
        require(block.timestamp - deployedAt >= withdrawalDelay, "Withdrawal delay has not passed yet");
        
        IERC20 token = IERC20(tokenContractAddress);
        uint256 balance = token.balanceOf(address(this));
        require(token.transfer(emergencyWallet, balance), "Token transfer failed");
    }
