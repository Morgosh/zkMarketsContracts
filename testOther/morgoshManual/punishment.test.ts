...
describe("ProphetsOfEthereum - Punishment Tests (Simplified)", () => {
  ...
  beforeEach(async () => {
    ...
    // Create wallets with known private keys for signing
    user1 = ethers.Wallet.createRandom().connect(provider);
    user2 = ethers.Wallet.createRandom().connect(provider);
    user3 = ethers.Wallet.createRandom().connect(provider);
    
    // Fund the wallets
    await deployer.sendTransaction({ to: user1.address, value: ethers.parseEther("10") });
    await deployer.sendTransaction({ to: user2.address, value: ethers.parseEther("10") });
    await deployer.sendTransaction({ to: user3.address, value: ethers.parseEther("10") });
    ...
    const mockMarketplaceArtifact = await hre.artifacts.readArtifact("MockMarketplace");
    const MockMarketplace = new ethers.ContractFactory(mockMarketplaceArtifact.abi, mockMarketplaceArtifact.bytecode, deployer);
    ...
    prophets = await Prophets.deploy(
      ...
      await mockMarketplace.getAddress(), // _marketplace
  ...
  describe("punishUnfaithful - Basic Validation", () => {
    ...
    beforeEach(async () => {
      ...
      // We'll compute the hash in each test after setting up orderParams
      signature = "0x" + "01".repeat(65); // Valid mock signature for testing
    });
    ...
    it("reverts if order is not for this collection", async () => {
      ...
      fullHash = await mockMarketplace.createOrderHash(orderParams);
        // Sign the raw hash directly (no message prefix for EIP-712 hash)
    ...
    it("reverts if offerer is not the token owner", async () => {
      ...
      fullHash = await mockMarketplace.createOrderHash(orderParams);
      // Sign the raw hash directly (no message prefix for EIP-712 hash)
      const signingKey2 = new ethers.SigningKey(user2.privateKey);
      signature = signingKey2.sign(fullHash).serialized;
      await expect(
        prophets.connect(user3).punishUnfaithful(orderParams, signature, fullHash)
  ...
  describe("Grace Period Protection Scenario", () => {
    it("demonstrates protection from retroactive punishment when floor rises", async () => {
      ...
      if (minimalFloor > 0n) {
        ...
        const fullHash = await mockMarketplace.createOrderHash(protectedOrderParams);
        // Should be protected from punishment due to grace period expiry
        const signingKey = new ethers.SigningKey(user1.privateKey);
        let signature = signingKey.sign(fullHash).serialized;
        await expect(
